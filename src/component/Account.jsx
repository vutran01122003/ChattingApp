...
const Account = ({
  ...
  const handleOpenConversation = () => {
    ...
    dispatch(
      ...
      navigation.navigate('ChatMessage', {conversationId: conversation_id});
    });
  };
        ...
        <View className="h-10 w-10 relative">
          ...
          {(isAdmin || isSubAdmin) && (
            <View className="h-5 w-5 rounded-full bg-stone-600/60 items-center justify-center absolute -bottom-0.5 -right-0.5">
              <Icon name="key" size={12} color={isAdmin ? 'yellow' : 'white'} />
            </View>
          )}
        </View>
